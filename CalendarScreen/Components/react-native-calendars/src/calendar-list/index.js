...
class CalendarList extends Component {
  ...
  constructor(props) {
    ...
    this.scrollEnd = _.debounce(this.scrollMomentEnd, 0);
    const rows = [];
    const texts = [];
  ...
  renderCalendar({ item }) {
    return (
      <CalendarListItem
        ...
        {...this.props}
        style={[this.props.calendarStyle, { paddingLeft: 0, paddingRight: 0, paddingTop: 0 }]}
      />
    );
  }
  ...
  onScrollEnd = (e) => {
    ...
    this.scrollEnd(pageNum);

  }
  scrollMomentEnd = (pageNum) => {
    if (!_.isString(this.state.rows[pageNum])) {
      this.props.monthChange(xdateToData(this.state.rows[pageNum]));
    ...
    return (
      <View>
        <FlatList
          ...
          initialNumToRender={this.props.horizontal ?
            this.getNumToRender(width, this.props.calendarWidth) :
            this.getNumToRender(height, this.props.calendarHeight)}
        />
